...
import uz.farobiy.lesson_11_backend.db.domain.customDto.student.StudentDto;
import uz.farobiy.lesson_11_backend.db.domain.customDto.student.StudentHomeworkDto;
...
public interface StudentRoleRepository extends JpaRepository<Homework, UUID> {
    ...
    @Query(value = "select * from " +
            ...
            "   f.name as homeworkFileName, f.path_url as homeworkPathUrl, f.size as homeworkFileSize from homeworks h " +
    ...
    @Query(nativeQuery = true,
            value = "select count(*) from (select * from tasks   " +
                    ...
                    "left join (select * from homeworks where student_id=:studentId) h on h.task_id=t.id " +
                    "where h.id IS NULL  " +
